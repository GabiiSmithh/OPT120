...
class _CadastroUsuarioAtividadeScreenState
  ...
  Future<void> _fetchAtividades() async {
    final response =
        await http.get(Uri.parse('http://localhost:3024/api/atividade'));
    if (response.statusCode == 200) {
      final List<dynamic> data = jsonDecode(response.body);
      setState(() {
    ...
    if (_formKey.currentState!.validate()) {
      try {
        final response = await http.post(
          Uri.parse('http://localhost:3024/api/usuario_atividade'),
          body: {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text(
          'Cadastro de Usuário Atividade',
          style: TextStyle(color: Colors.white),
        ),
        iconTheme: const IconThemeData(color: Colors.white),
      ),
      body: Container(
        color: Colors.teal, // Background color changed to teal
        child: Padding(
